...
c_screen_virtual_keyboard::c_screen_virtual_keyboard(
	...
	c_screen_virtual_keyboard(parameters->get_channel_type(), parameters->get_window_index(), parameters->get_user_flags())
{
	return;
}

c_screen_virtual_keyboard::c_screen_virtual_keyboard(
	e_user_interface_channel_type _channel_type,
...
{
	void* pool;
	c_screen_virtual_keyboard* screen = (pool = ui_pool_allocate_space(sizeof(c_screen_virtual_keyboard), 0)) ? new (pool) c_screen_virtual_keyboard(parameters) : NULL;
	ASSERT(screen != NULL);
...
void* ui_load_virtual_keyboard(wchar_t* out_keyboard_text, uint32 out_keyboard_text_length, e_vkbd_context_type keyboard_type)
{
	...
	c_screen_virtual_keyboard* virtual_keyboard = (c_screen_virtual_keyboard*)virtual_keyboard_params.execute_load_function();
