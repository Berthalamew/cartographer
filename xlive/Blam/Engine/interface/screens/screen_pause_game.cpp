...
void c_pause_game_list::update_list_items(c_list_item_widget* item, int32 skin_index)
{
	ASSERT(item!=NULL);
...
void c_screen_pause_game::initialize(c_screen_parameters* parameters)
{
	c_screen_with_menu::initialize(parameters);
	return;
}

void c_screen_pause_game::update_objective_text(
	void)
{
	...
	s_event_record* const& event)
{
	...
	params.initialize_default_user(FLAG(event->controller), get_channel(), get_render_window(), NULL);
