...
public:
	...
	void initialize(int32 view_index, class c_replication_entity_manager* entity_manager);
	void reset(void);

	void create_entity(int32 entity_index);
	void set_entity_dirty(int32 entity_index, uint32 update_mask);

	void mark_entity_for_deletion(int32 entity_index);
...
private:
	...
	bool write_creation_to_packet(int32 request_identifier, void const* telemetry_data, class c_bitstream* packet, int32 must_leave_space_bits);
	s_replication_entity_view_data* get_entity(int32 entity_index);

	/* Friend functions for other classes that need to access our privates */

	friend class c_replication_entity_manager;
